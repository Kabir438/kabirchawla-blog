    ...
    return (
        <div className="flex justify-center w-min space-x-2 relative [user-select:none]" onMouseLeave={() => setHoverStatus(null)}>
            {[1, 2, 3, 4, 5].map((rating) => (
                <Button onMouseDown={onClick} onMouseUp={onClick} key={`rating-${rating}`} variant="ghost" className={cn("rounded-full icon-glow group/button relative hover:bg-transparent overflow-hidden h-16 w-16", (hoverStatus && isActive && (hoverStatus === (rating - 0.5) || hoverStatus === rating)) ? "active" : "")} size="icon">
                    {/* full present */}
                    <Star
                        className={cn("w-[calc(100%_-_28px)] h-[calc(100%_-_28px)] transition-all duration-200 text-amber-500 z-0 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2",
                            ((hoverStatus !== null && !isActive) && hoverStatus >= (rating)) ? "opacity-100" : "opacity-0"
                        )}
                    />
                    {/* none present */}
                    <Star
                        className={cn("w-[calc(100%_-_28px)] h-[calc(100%_-_28px)] transition-all duration-200 text-gray-500 z-0 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2",
                    ...
                    <Star 
                        className={cn("w-[calc(100%_-_28px)] h-[calc(100%_-_28px)] transition-all duration-200 text-white z-0 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2",
                            ((hoverStatus === null) && pressedStatus === null) ? "opacity-100" : "opacity-0"
                        )} 
                    />
                    {/* not hovered && full selected */}
                    <Star 
                        className={cn("w-[calc(100%_-_28px)] h-[calc(100%_-_28px)] transition-all duration-200 lucide-filled-star text-amber-400 z-0 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2",
                    ...
                    <Star
                        className={cn("w-[calc(100%_-_28px)] h-[calc(100%_-_28px)] transition-all duration-200 lucide-filled-star text-gray-500 z-0 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2",
                    ...
                    <Star
                        className={cn("w-[calc(100%_-_28px)] h-[calc(100%_-_28px)] transition-all duration-200 lucide-half-filled-star text-amber-400 z-0 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2",
                            ((hoverStatus === null || isActive) && (isActive ? (typeof hoverStatus === "number" ? hoverStatus : pressedStatus) : pressedStatus) !== null && (isActive ? (typeof hoverStatus === "number" ? hoverStatus : pressedStatus) : pressedStatus)  === (rating - 0.5)) ? "opacity-100" : "opacity-0"
                        )} 
                    />
